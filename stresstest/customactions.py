...
class CustomActions():
    ...
    def swipeandhold(self, start_x: int, start_y: int, end_x: int, end_y: int, duration: int = 0,
                     holdduration: int = 0) -> 'WebDriver':
        ...
        actions.w3c_actions.pointer_action.pause(holdduration / 1000)
        actions.w3c_actions.pointer_action.release()
        actions.perform()
        return self  # type: ignore
    ...
    def longpress(self, elementlc_x, elementlc_y, locator, duration: int = 1000) -> 'WebDriver':
        """Swipe from one point to another point, for an optional duration.

             Args:
                 locator: element location.
                 duration: to hold in that point

             Usage:
                 driver.longpress(elementid, 1000)
        ...
        actions = ActionChains(self)
        actions.w3c_actions = ActionBuilder(self, mouse=PointerInput(interaction.POINTER_TOUCH, "touch"))
        actions.w3c_actions.pointer_action.move_to_location(elementlc_x, elementlc_y)
        actions.w3c_actions.pointer_action.pointer_down()
        actions.w3c_actions.pointer_action.pause(duration / 1000)
        actions.w3c_actions.pointer_action.release()
        actions.perform()
